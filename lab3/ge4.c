        ...
        for (i = k+1; i < SIZE; i++) {
            ...
            for(j=k+1;j<SIZE;){
                if (j<max(SIZE-BLCKSZ, 0)){
                    A[i][j] = A[i][j]-A[k][j]*division;
                    A[i][j+1] = A[i][j+1]-A[k][j+1]*division;
                    A[i][j+2] = A[i][j+2]-A[k][j+2]*division;
                    A[i][j+3] = A[i][j+3]-A[k][j+3]*division;
                    A[i][j+4] = A[i][j+4]-A[k][j+4]*division;
                    A[i][j+5] = A[i][j+5]-A[k][j+5]*division;
                    A[i][j+6] = A[i][j+6]-A[k][j+6]*division;
                    A[i][j+7] = A[i][j+7]-A[k][j+7]*division;
                    j += BLCKSZ;
                }else{
                    A[i][j] = A[i][j]-A[k][j]*division;
                    j++;
