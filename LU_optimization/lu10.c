        ...
        for (j = i + 1; j < SIZE; j++) {
            ...
            for (k=i+1;k<SIZE;){
                if (k < max(SIZE - BLKSZ, 0)){
                    ...
                    tmp4 = _mm256_loadu_pd(A + IDX(jIdx,k+8));
                    tmp6 = _mm256_loadu_pd(A + IDX(jIdx,k+12));

                    tmp1 = _mm256_loadu_pd(A + IDX(iIdx,k));
                    tmp3 = _mm256_loadu_pd(A + IDX(iIdx,k+4));
                    tmp5 = _mm256_loadu_pd(A + IDX(iIdx,k+8));
                    tmp7 = _mm256_loadu_pd(A + IDX(iIdx,k+12));
                    ...
                    _mm256_storeu_pd(A + IDX(jIdx,k+8),tmp4);
                    _mm256_storeu_pd(A + IDX(jIdx,k+12),tmp6);
                    k += BLKSZ;
                }else{
