...
int LUPDecompose(double *A, register int SIZE, double Tol, int *P) {
    ...
    for (i = 0; i < SIZE; i++) {
        ...
        for (j = i + 1; j < SIZE; j++) {
            ...
            mm_multiplier[0] = mm_multiplier[1] = multiplier;

            for (k=i+1;k<SIZE;){
                if (k < max(SIZE - BLKSZ, 0)){
                    tmp0 = _mm_loadu_pd(A + IDX(jIdx,k));
                    tmp2 = _mm_loadu_pd(A + IDX(jIdx,k+2));
                    tmp4 = _mm_loadu_pd(A + IDX(jIdx,k+4));
                    tmp6 = _mm_loadu_pd(A + IDX(jIdx,k+6));

                    tmp1 = _mm_loadu_pd(A + IDX(iIdx,k));
                    tmp3 = _mm_loadu_pd(A + IDX(iIdx,k+2));
                    tmp5 = _mm_loadu_pd(A + IDX(iIdx,k+4));
                    tmp7 = _mm_loadu_pd(A + IDX(iIdx,k+6));
                    ...
                    _mm_storeu_pd(A + IDX(jIdx,k),tmp0);
                    _mm_storeu_pd(A + IDX(jIdx,k+2),tmp2);
                    _mm_storeu_pd(A + IDX(jIdx,k+4),tmp4);
                    _mm_storeu_pd(A + IDX(jIdx,k+6),tmp6);
                    k += BLKSZ;
                }else{
